...
start_adg(){
	...
	run_adguardhome="/usr/bin/AdGuardHome/AdGuardHome"
	if [ ! -f $run_adguardhome ]; then
	run_adguardhome="/tmp/AdGuardHome/AdGuardHome"
	dl_adg
	fi
	getconfig
	change_dns
	...
	eval "$run_adguardhome -c $adg_file -w /tmp/AdGuardHome -v" &
