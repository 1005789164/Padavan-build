...
setTimeout('document.getElementById("btn_ctime").style.display="none";',1000);
		...
		function showsudlinkList() {
			var is_same = document.getElementById("u_nodeList").value;
			if (is_same == "same")
				document.getElementById("ud_type").value = document.getElementById("d_type").value;
			else{
				var key = "ssconf_basic_json_" + is_same;
				var result = JSON.parse(db_ss[key]);
		...
		function china_dns_changed(){
			if (document.form.china_dns.value == "1"){
				showhide_div('id_china_dns_input',1);
			}
			else{
				showhide_div('id_china_dns_input',0);
			}
		}
		function tunnel_forward_changed(){
			if (document.form.tunnel_forward.value == "1"){
				showhide_div('id_tunnel_forward_input',1);
			}
			else{
				showhide_div('id_tunnel_forward_input',0);
			}
		}
	</script>
	<style>
...
<select name="china_dns" class="input" onchange="china_dns_changed();"
																style="width: 200px;">
...
<option value="1">SmartDns(仅绕过模式生效)</option>
															</select>

<span id="id_china_dns_input" style="display:none;">
<input type="text" maxlength="64" class="input" size="64" name="ss_sdns_port" style="width: 190px;" value="<% nvram_get_x("", "ss_sdns_port"); %>">
</span>
														</td>
													</tr>
...
<select name="tunnel_forward" class="input" onchange="tunnel_forward_changed();"
																style="width: 200px;">
...
<span id="id_tunnel_forward_input" style="display:none;">
<input type="text" maxlength="64" class="input" size="64" name="ss_sdnse_port" style="width: 190px;" value="<% nvram_get_x("", "ss_sdnse_port"); %>">
</span>
														</td>
													</tr>
													<!--
